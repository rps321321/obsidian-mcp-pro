...
- **Untrusted vault text boundaries** — tool outputs that include note bodies, read, search result, semantic result, write rewrite-warning, link-graph, attachment, Base, and Canvas path summaries, search snippets, semantic snippets and heading paths, tag lists, tag search result paths and previews, frontmatter values, Base data, wikilink targets in link-analysis output, SVG attachment text, section headings, Canvas node previews and edge labels, or backlink context wrap those vault-authored portions in `[BEGIN UNTRUSTED VAULT CONTENT: ...]` / `[END UNTRUSTED VAULT CONTENT: ...]` markers before they enter an MCP client's model context. Note and daily resources preserve raw markdown text for fidelity, but carry `_meta["obsidian-mcp-pro/contentTrust"] = "untrusted-vault-content"`.
