...
- **YAML parser boundaries** — Obsidian properties are parsed only from `---` YAML delimiter lines; non-YAML language blocks (e.g. `---js`) stay as body text, oversized frontmatter is skipped on reads and refused for metadata updates, and note/Base YAML containing anchors or aliases is not parsed. Frontmatter is parsed and emitted with js-yaml directly, so no extra YAML wrapper dependency ships with the server.
