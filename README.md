...
- **Untrusted vault text boundaries** — tool outputs that include note bodies, search snippets, semantic snippets and heading paths, tag previews, frontmatter values, Base data, SVG attachment text, section headings, Canvas node previews, or backlink context wrap those vault-authored portions in `[BEGIN UNTRUSTED VAULT CONTENT: ...]` / `[END UNTRUSTED VAULT CONTENT: ...]` markers before they enter an MCP client's model context. Note and daily resources preserve raw markdown text for fidelity, but carry `_meta["obsidian-mcp-pro/contentTrust"] = "untrusted-vault-content"`.
