...
describe("base handlers — read_base", () => {
  ...
  it("escapes control characters in parsed display fields", async () => {
    ...
    expect(text).toContain("[BEGIN UNTRUSTED VAULT CONTENT: read_base document]");
    expect(text).toContain("dirty.base");
    expect(text).toContain("bad\\tkey");
    expect(text).toContain("Display\\nName");
    expect(text).toContain("Main\\nView");
    expect(text).toContain("table\\tview");
    expect(text).not.toContain("[BEGIN UNTRUSTED VAULT CONTENT: base: dirty.base]");
...
describe("base handlers — query_base", () => {
  ...
  it("escapes control characters in view labels and frontmatter keys", async () => {
    ...
    expect(text).toContain("[BEGIN UNTRUSTED VAULT CONTENT: query_base row path]");
    expect(text).toContain("[BEGIN UNTRUSTED VAULT CONTENT: query_base row frontmatter]");
    expect(text).toContain("- note.md");
    expect(text).toContain('    bad\\tkey: "dirty\\nvalue"');
    expect(text).not.toContain("[BEGIN UNTRUSTED VAULT CONTENT: query_base row path: note.md]");
    expect(text).not.toContain("[BEGIN UNTRUSTED VAULT CONTENT: base row frontmatter: note.md]");
