  ...
  defineTool(
    ...
    vaultPath,
    {
      ...
      inputSchema: {
        path: z
          ...
          .max(500)
          .describe(
            "Relative path from vault root to the note (e.g., 'projects/alpha.md'). Extension optional."
          ),
        properties: z
          .string()
          .max(100_000)
          .describe(
            'JSON object string of frontmatter keys to set, e.g., \'{"status":"done","priority":1,"tags":["review"]}\'. Existing keys not in the payload are preserved.'
          ),
      },
    },
    async ({ path: notePath, properties }) => {
      ...
      try {
        ...
        return error("Error: Invalid JSON in properties parameter.");
      }
