...
- Semantic search outputs now mark ranked note result paths from `search_semantic` and `find_similar_notes` as untrusted vault content.
- Tag outputs now mark matching note paths from `search_by_tag` and skipped note rows from `rename_tag` as untrusted vault content.
- Write-tool rewrite warnings now mark failed referrer paths from `move_note` and `delete_note` as untrusted vault content.
...
- Canvas inventory and read outputs now mark listed Canvas paths and read Canvas paths as untrusted vault content.
