...
- `get_note`, `get_daily_note`, `search_notes`, `search_by_frontmatter`, `search_by_tag includeContent`, `search_semantic includeSnippet`, semantic heading paths, `read_canvas`, `read_base`, `query_base includeFrontmatter`, SVG attachment text, section-heading output, and backlink context output now wrap vault-authored text in `[BEGIN UNTRUSTED VAULT CONTENT: ...]` / `[END UNTRUSTED VAULT CONTENT: ...]` markers. Clients that parsed raw note fragments, headings, or snippets should extract the text between those markers. Raw markdown resources still return the original note text, but now carry `_meta["obsidian-mcp-pro/contentTrust"] = "untrusted-vault-content"`.
